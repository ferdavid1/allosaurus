...
def read_recognizer(inference_config_or_name='latest', alt_model_path=None):
    ...
    if isinstance(inference_config_or_name, str):
        ...
        inference_config = Namespace(model=model_name, device_id=-1, lang='eng', approximate=False, prior=None)
    else:
...
class Recognizer:
    ...
    def recognize(self, filename, lang_id='eng', topk=1, emit=1.0, timestamp=False, phoneme=False):
    ...
    def get_logits(self, filename, lang_id='eng'):
